...
export function NestedSidebarItem({
  ...
  // Shared classes for every item
  const base =
    "flex items-center transition-colors hover:bg-muted dark:hover:bg-slate-800";
  ...
  // Simple link when there are no sub-items
  if (!subItems?.length) {
    return (
      <NavLink
        to={href || "#"}
  ...
  // Collapsible section when there are sub-items
  return (
    <Collapsible
